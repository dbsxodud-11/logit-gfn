  ...
  class TFBind8MDP(base):
    def __init__(self, args):
      ...
      if args.task == "offline_generalization":
        threshold = np.percentile(py, q=50)
        self.offline_dataset = {x: y for x, y in self.scaled_oracle.items() if y <= threshold}
    ...
    def make_monitor(self):
      ...
      rs_all = list(self.oracle.values())
      target.init_from_base_rewards(rs_all, target_beta=self.args.target_beta)
...
def offline_generalization(args):
  print("Offline generalization in tfbind8 ...")
