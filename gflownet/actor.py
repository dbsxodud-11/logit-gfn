from .policy import BasePolicySA, BasePolicySSR, CondPolicySSR
...
class Actor:
  ...
  """
    Construct policy
  """
  def make_policy(self, direction):
    ...
    if self.args.sa_or_ssr == 'ssr':
      if self.args.temp_cond:
        if self.args.temp_cond_type == "logit":
          return CondPolicySSR(self.args, self.mdp, actor = self,
                               encoder = self.net_encoder_ssr(),
                               scorer = self.net_scorer_ssr(),
                               state_map_f = state_map_f,
                               temp_encoder = self.net_temp_encoder(),
                               temp_decoder = self.net_temp_decoder(),
                              )
        else:
          return CondPolicySSR(self.args, self.mdp, actor = self,
                               encoder = self.net_encoder_ssr(),
