...
class SeqPAActor(Actor):
  ...
  def __init__(self, args, mdp):
    ...
    self.policy_fwd = super().make_policy('forward')
    self.policy_back = super().make_policy('backward')
  ...
  def net_scorer_ssr(self):
    """ [encoding1, encoding2, (temp)] -> scalar"""
    hid_dim = self.args.ssr_scorer_hid_dim
    n_layers = self.args.ssr_scorer_n_layers
    ssr_embed_dim = self.args.ssr_embed_dim
    
    if self.args.temp_cond_type == "logit" and self.args.layer_conditioning:
      temp_embed_dim = self.args.ssr_temp_cond_hid_dim
    elif self.args.temp_cond_type == "layer":
      temp_embed_dim = self.args.ssr_temp_cond_hid_dim
    else:
      temp_embed_dim = None

    if self.args.temp_cond == False or temp_embed_dim is None:
      ...
      return network.make_mlp(
          [2*ssr_embed_dim + temp_embed_dim] + \
          [hid_dim] * n_layers + \
          [1]
      )
      
  def net_temp_encoder(self):
    hid_dim = self.args.ssr_temp_cond_hid_dim
    n_layers = self.args.ssr_temp_cond_n_layers
