...
class BasePolicySSR:
  ...
  def parameters(self):
    """ Retrieve trainable parameters, send to optimizer. """
    return chain(*[net.parameters() for net in self.nets])
  ...
  def load_state_dict(self, state_dicts):
    for net, state_dict in zip(self.nets, state_dicts):
  ...
  def __forward(self, state, childs_uniq):
    ...
    embed_state = embeds[0]
    embed_children = embeds[1:]

    # Score [z_state, z_i] for each i
    ssr_inp = lambda embed_child: torch.cat((embed_state, embed_child))
    ...
    logps_uniq = torch.clamp(logps_uniq, self.args.clip_policy_logit_min,
                                           self.args.clip_policy_logit_max)
    return childs_uniq, logps_uniq


class CondPolicySSR(BasePolicySSR):
  """ Temperature-conditional policy class - inherited and specified in MDPs. 

      SSR: State x state x beta -> R (log energy)

      A policy is a deep neural net that samples actions from states.
      The network architecture depends on the specific MDP.

      CondTBGFlowNet objects contain two Policy objects - forward and 
      backward - and temperatural-conditional partition function with own the optimizers and training logic for the
      Policies.
  """
  def __init__(self, args, mdp, actor, encoder, scorer, state_map_f, temp_encoder=None, temp_decoder=None):
    super().__init__(args, mdp, actor, encoder, scorer, state_map_f)
    
    self.temp_encoder = temp_encoder
    self.temp_decoder = temp_decoder

    self.nets.append(self.temp_encoder)
    if self.temp_decoder is not None:
      self.nets.append(self.temp_decoder)
  ...
  """
    logps unique
  """
  def logps_unique(self, batch, temp=None):
    batched = bool(type(batch) is list)
    if not batched:
      batch = [batch]

    if self.parallelize:
      res = self.parallel_logps_unique(batch, temp=temp)
    else:
      res = self.serial_logps_unique(batch, temp=temp)
    return res if batched else res[0]
        
  def parallel_logps_unique(self, batch, temp=None):
    batch_dicts = []
    futures = []
    for state in batch:
      fut = unique_keep_order_filter_children.remote(state, self.ray_state_map_f)
    ...
    results = ray.get(futures)
    for childs_uniq in results:
      childs_uniq, logps_uniq = self.__forward(state, childs_uniq, temp=temp)
      state_to_logp = {child: logp
                       for child, logp in zip(childs_uniq, logps_uniq)}
      batch_dicts.append(state_to_logp)
    return batch_dicts
  
  def serial_logps_unique(self, batch, temp=None):
    batch_dicts = []
    for i, state in enumerate(batch):
      children = self.state_map_f(state)
      childs_uniq = unique_keep_order_remove_nones(children)
      
      childs_uniq, logps_uniq = self.__forward(state, childs_uniq, temp=temp[i])
      state_to_logp = {child: logp for child, logp in zip(childs_uniq, logps_uniq)}
      batch_dicts.append(state_to_logp)
    return batch_dicts
  ...
  """
    sample
  """
  def sample(self, batch, epsilon=0.0, temp=None):
    """ Non-differentiable; sample a child or parent.
    ...
    if self.parallelize:
      res = self.parallel_sample(batch, epsilon=epsilon, temp=temp)
    else:
      res = self.serial_sample(batch, epsilon=epsilon, temp=temp)
    return res if batched else res[0]

  def parallel_sample(self, batch, epsilon=0.0, temp=None):
    batch_samples = []
    futures = []
    for state in batch:
      fut = unique_keep_order_filter_children.remote(state, self.ray_state_map_f)
  ...
  def serial_sample(self, batch, epsilon=0.0, temp=None):
    batch_samples = []
    for i, state in enumerate(batch):
      children = self.state_map_f(state)
      # Reduce children to valid, unique
  ...
  """
    Forward
  """
  def __forward(self, state, childs_uniq, temp=None):
    """ Single state -> (unique child states, logps) efficiently.
    ...
    embed_temp = self.temp_encoder(temp)
    if self.args.temp_cond_type == "logit":
      if self.args.layer_conditioning:
        ssr_inp = lambda embed_child: torch.cat((embed_state, embed_child, embed_temp))
        scorer_inp = torch.stack([ssr_inp(e_child) for e_child in embed_children])
      else:
        ssr_inp = lambda embed_child: torch.cat((embed_state, embed_child))
        scorer_inp = torch.stack([ssr_inp(e_child) for e_child in embed_children])
      ...
      scores_uniq = torch.squeeze(self.scorer(scorer_inp), -1)
      # Logit-Scaling Trick Applies Here
      scores_uniq = self.temp_decoder(embed_temp) * scores_uniq
    elif self.args.temp_cond_type == "layer":
      ssr_inp = lambda embed_child: torch.cat((embed_state, embed_child, embed_temp))
      scorer_inp = torch.stack([ssr_inp(e_child) for e_child in embed_children])
    ...
    scores_uniq = torch.clamp(scores_uniq, self.args.clip_policy_logit_min,
                                           self.args.clip_policy_logit_max)
    logps_uniq = scores_uniq - torch.logsumexp(scores_uniq, -1)
    return childs_uniq, logps_uniq
