...
class CondTBGFlowNet():
  ...
  def __init__(self, args, mdp, actor):
    ...
    if self.args.temp_cond_type == "layer" and self.args.thermometer:
      self.logZ = network.make_mlp(
        [self.args.num_thermometer_dim] + \
        [self.args.ssr_temp_cond_hid_dim] * self.args.ssr_temp_cond_n_layers + \
        [1]
      )
    else:
      self.logZ = network.make_mlp(
    ...
    self.nets = [self.policy_fwd, self.policy_back, self.logZ]
    
    for net in self.nets:
      net.to(args.device)

    self.clip_grad_norm_params = [list(self.policy_fwd.parameters()),
                                  list(self.policy_back.parameters()),
                                  list(self.logZ.parameters())]
...
#     return
  ...
  """
  def fwd_logps_unique(self, batch, temp):
    ...
    return self.policy_fwd.logps_unique(batch, temp)
  
  def fwd_logfs_unique(self, batch, temp=None):
    return self.logF(batch, temp=temp)

  def fwd_sample(self, batch, epsilon=0.0, temp=None):
    ...
    """
    return self.policy_fwd.sample(batch, epsilon=epsilon, temp=temp)

  def back_logps_unique(self, batch, temp):
    """ Differentiable; output logps of unique children/parents. """
    return self.policy_back.logps_unique(batch, temp)
  
  def back_logfs_unique(self, batch, temp=None):
    return self.logF(batch, temp=temp)

  def back_sample(self, batch, temp=None):
    """ Non-differentiable; sample a child or parent.

        Input: batch: List of [State], or State
        Output: List of [State], or State
    """
    return self.policy_back.sample(batch, temp=temp)

  """
    Exploration & modified policies
  """
  def batch_fwd_sample(self, n, epsilon=0.0, uniform=False, temp=None):
    ...
    while len(incomplete_trajs) > 0:
      ...
      samples = self.fwd_sample(inp, epsilon=epsilon, temp=temp)
  ...
  def batch_back_sample(self, xs, temp=None):
    ...
    while len(incomplete_trajs) > 0:
      ...
      samples = self.back_sample(inp, temp=temp)
      for i, sample in enumerate(samples):
  ...
  """
    Trajectories
  """
  def traj_fwd_logp(self, exp, temp=None):
    """ Computes logp(trajectory) under current model.
        Batches over states in trajectory. 
    """
    states_to_logps = self.fwd_logps_unique(exp.traj[:-1], temp=temp)
    total = 0
  ...
  def traj_back_logp(self, exp, temp=None):
    """ Computes logp(trajectory) under current model.
        Batches over states in trajectory. 
    """
    states_to_logps = self.back_logps_unique(exp.traj[1:], temp=temp)
    total = 0
    for state_to_logp, parent in zip(states_to_logps, exp.traj[:-1]):
      total += state_to_logp[parent]
    return total

  def batch_traj_fwd_logp(self, batch, temp=None):
    """ Computes logp(trajectory) under current model.
    ...
    logZ = self.logZ(temp["beta_enc"])
    fwd_chain = logZ.squeeze(-1)
    
    temp = temp["beta_enc"].repeat_interleave(self.args.forced_stop_len+1, dim=0)
    states_to_logps = self.fwd_logps_unique(fwd_states, temp=temp)
    ...
    # fwd chain is [bsize]
    # print(fwd_chain.shape)
    return fwd_chain
  
  def batch_traj_fwd_logp_unroll(self, batch, temp=None):
    trajs = [exp.traj for exp in batch]
    fwd_states, back_states, unroll_idxs = unroll_trajs(trajs)
    
    temp = temp["beta_enc"].repeat_interleave(self.args.forced_stop_len+1, dim=0)
    states_to_logfs = self.fwd_logfs_unique(fwd_states, temp=temp)
    states_to_logps = self.fwd_logps_unique(fwd_states, temp=temp)
    fwd_logp_chosen = [s2lp[c] for s2lp, c in zip(states_to_logps, back_states)]
  ...
  def batch_traj_back_logp(self, batch, temp=None):
    """ Computes logp(trajectory) under current model.
        Batches over all states in all trajectories in a batch.
    ...
    back_chain = temp["beta"].squeeze(-1) * torch.stack([exp.logr for exp in batch])
    
    temp = temp["beta_enc"].repeat_interleave(self.args.forced_stop_len+1, dim=0)
    states_to_logps = self.back_logps_unique(back_states, temp=temp)
    ...
    # back_chain is [bsize]
    # print(back_chain.shape)
    return back_chain
  
  def batch_traj_back_logp_unroll(self, batch, temp=None):
    trajs = [exp.traj for exp in batch]
    fwd_states, back_states, unroll_idxs = unroll_trajs(trajs)
    
    temp = temp["beta_enc"].repeat_interleave(self.args.forced_stop_len+1, dim=0)
    states_to_logfs = self.back_logfs_unique(back_states, temp=temp)
    states_to_logps = self.back_logps_unique(back_states, temp=temp)
    back_logp_chosen = [s2lp[p] for s2lp, p in zip(states_to_logps, fwd_states)]
  ...
  """
    Learning
  """
  def batch_loss_trajectory_balance(self, batch, temp=None):
    ...
    """
    fwd_chain = self.batch_traj_fwd_logp(batch, temp)
    back_chain = self.batch_traj_back_logp(batch, temp)
  ...
  def train_tb(self, batch, log=True, temp=None, online=False):
    ...
    """
    batch_loss = self.batch_loss_trajectory_balance(batch, temp=temp)
  ...
  def save_params(self, file):
    ...
    #   }, file)
    # elif self.args.model == "cond_db" or self.args.model == "cond_subtb0.9":
    #   torch.save({
    #     'policy_fwd':   self.policy_fwd.state_dict(),
    #     'policy_back':  self.policy_back.state_dict(),
    #     'logZ':         self.logZ,
  ...
  def load_for_eval_from_checkpoint(self, file):
    ...
    self.logZ.load_state_dict(checkpoint['logZ'])
    for net in self.nets:
      net.eval()
    return
